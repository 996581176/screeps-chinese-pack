  ...
  return allQueryContents.filter(content => {
    ...
    targetElements.forEach((element, index) => {
      ...
      if (!content.ingnoreRepeatedCheck) {
        ...
        const newContent = content.protect ? updateProtectElement(element, content) : updateElement(element, content);
...
const translateNormalContent = function (el: Node, allContents: TranslationContent[]): TranslationContent[] {
  ...
  needTranslateText.forEach(text => {
    ...
    // 找到符合的翻译内容，并保存其索引
    let translationIndex: number | undefined;
    ...
    if (!currentTranslation.reuse && translationIndex !== undefined) allContents.splice(translationIndex, 1);
  });
